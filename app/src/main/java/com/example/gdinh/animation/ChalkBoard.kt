...
(context: Context) : View(context) {
    ...
    private var style = BOUNCE_ROTATE  //style holds current type of animation
    ...
    fun wander() {
        ...
        when (style) {
            ...
            BOUNCE_ROTATE -> {
                val together = AnimatorSet()
                val bounceGuy = getObjectAnimator(500, "fraction", 0.0f, 1.0f) //local method
                anim = getObjectAnimator(700, "angle", 0.0f, 360.0f)
                anim.interpolator = BounceInterpolator()
                together.play(bounceGuy).with(anim)
                together.start()
            ...
            MOVE_BOUNCE -> {
                ...
                anim.interpolator = BounceInterpolator()
                together.play(anim).after(mover)
                together.start()
            }
            else -> {
            }
    ...
    companion object {  //static stuff
        ...
        const val BOUNCE_ROTATE = 24
        const val MOVE_BOUNCE = 25
